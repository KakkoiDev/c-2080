...
Source lives in `src/index.md` (markdown) and `src/template.html` (page shell). `build.mjs` runs markdown-it with custom containers (`::: rule | warn | tip | pattern | grid | item`) and writes `index.html` for GitHub Pages. CI verifies the committed `index.html` is up-to-date.
